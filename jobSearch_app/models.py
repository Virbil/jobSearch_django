...
class FormManager(models.manager):
    ...
    def create_job_validator(self, postData):
        ...
        if len(postData['company']) < 1:
            errors['company_empty'] = "cannot be blank"
        if len(postData['company']) < 50:
            errors['company_long'] = "cannot be longer than 50 characters" 

        if len(postData['job_title']) < 1:
            errors['job_title_empty'] = "cannot be blank"
        if len(postData['job_title']) < 50:
            errors['job_title_long'] = "cannot be longer than 50 characters" 
        ...
        if len(postData['post_date']) < 1:
            errors['post_date_empty'] = "cannot be blank"


        if len(postData['job_url']) < 1:
            errors['job_url_empty'] = "cannot be blank"

        if len(postData['min']) < 1:
            errors['min_empty'] = "cannot be blank"
            
        if len(postData['max']) < 1:
            errors['max_empty'] = "cannot be blank"

        if len(postData['summary']) < 1:
            errors['summary_empty'] = "cannot be blank"
        if len(postData['summary']) < 255:
            errors['summary_long'] = "cannot be longer than 255 characters"

        if len(postData['description']) < 1:
            errors['description_empty'] = "cannot be blank"
